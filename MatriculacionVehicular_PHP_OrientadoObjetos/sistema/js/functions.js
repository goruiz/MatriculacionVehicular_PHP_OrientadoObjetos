$(document).ready(function(){
	...
	$("#btn_create_user").click(function (e) {
		...
		if($('#nombre').val() == ''  || $('#user').val()=='' || $('#password').val()=='')
		{
			$('#alert').html('<p class="error">Completa los campos mijin.</p>');
			$('#alert').show(1000);
			return false;
		}
	    ...
	    $.ajax({
		        ...
		        success: function (response) {
		        	...
		        	if(response == 'save')
		        	{
		        		$('#popup_register').html('<div id="alert_user_create"><a id="close_form" href="./">X</a><h2>Usuario Creado</h2><p class="save">Usuario creado con Ã©xito, por favor sigue las instrucciones que te  han enviado al correo electrÃ³nico.</p></div>');
		        	}
		        },
